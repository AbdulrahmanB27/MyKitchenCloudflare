...
export const onRequestGet: PagesFunction<Env> = async (context) => {
  ...
  try {
    ...
    const { results } = await context.env.DB.prepare("SELECT data FROM meal_plans WHERE family_id = ?").bind(familyId).all();
...
export const onRequestPost: PagesFunction<Env> = async (context) => {
  ...
  try {
    ...
    ).bind(plan.id, familyId, plan.date, plan.slot, plan.recipeId, JSON.stringify(plan), Date.now()).run();
...
export const onRequestDelete: PagesFunction<Env> = async (context) => {
  ...
  try {
    ...
    // Verify ownership
    const existing = await context.env.DB.prepare("SELECT family_id FROM meal_plans WHERE id = ?").bind(id).first();
