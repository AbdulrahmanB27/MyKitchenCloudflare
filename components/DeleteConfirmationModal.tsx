...
const DeleteConfirmationModal: React.FC<DeleteConfirmationModalProps> = ({ itemName, isOpen, onClose, onConfirm }) => {
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-background-dark/80 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-white dark:bg-card-dark rounded-2xl shadow-xl w-full max-w-md border border-border-thin dark:border-border-dark overflow-hidden flex flex-col">
        <div className="p-6">
            ...
            {isMultiFamily && (
                <div className="mt-6 space-y-3">
                    <div className="flex items-center justify-between">
                        <p className="text-xs font-bold uppercase text-text-secondary">Delete from:</p>
                        <button 
                            onClick={toggleAll}
                            className="text-xs font-bold text-forest-green dark:text-accent-herb hover:underline"
                        >
                                ...
                                <button 
                                    ...
                                    className={`w-full flex items-center justify-between p-3 rounded-lg border transition-all ${
                                        isSelected 
                                            ...
                                            : 'border-border-thin dark:border-border-dark hover:bg-bg-subtle dark:hover:bg-white/5'
                                    }`}
                                >
                                    <div className="flex flex-col items-start">
                                        ...
                                        {isCurrent && <span className="text-[10px] text-text-secondary uppercase font-bold">Current Family</span>}
                                    </div>
